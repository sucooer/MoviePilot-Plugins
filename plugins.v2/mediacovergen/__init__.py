...
class MediaCoverGen(_PluginBase):
    ...
    def get_api(self) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            {
                "path": "/apply_saved_cover",
                "endpoint": self.api_apply_saved_cover,
                "auth": "bear",
                "methods": ["POST", "GET"],
                "summary": "将一张历史封面设为当前封面",
            },
            {
                "path": "delete_saved_cover",
                "endpoint": self.api_delete_saved_cover,
    ...
    def api_apply_saved_cover(self, file: str = ""):
        try:
            ...
            cover_meta = self.__get_saved_cover_metadata(str(target_file))
            if not cover_meta:
                return {"code": 1, "msg": "未找到该历史封面对应的媒体库信息"}

            service = self.__get_service_by_name(cover_meta.get("server"))
            if not service:
                return {"code": 1, "msg": f"未找到媒体服务器：{cover_meta.get('server')}"}

            library = self.__find_library_by_id(service, str(cover_meta.get("library_id")))
            if not library:
                return {"code": 1, "msg": f"未找到媒体库：{cover_meta.get('library_name') or cover_meta.get('library_id')}"}

            image_base64 = self.__read_file_as_base64(target_file)
            if not image_base64:
...
''',
    ...
    def __style_preview_src(index: int) -> str:
        safe_index = max(1, min(4, int(index)))
        return f"https://raw.githubusercontent.com/sucooer/MoviePilot-Plugins/main/images/style_{safe_index}.jpeg"
    ...
    def __save_cover_file_metadata(self, file_path: str, server_name: str, library_name: str, library_id: Optional[str]):
        try:
            history = self.get_data("saved_cover_meta") or {}
            ...
            self.save_data("saved_cover_meta", history)
        except Exception as e:
            logger.warning(f"保存历史封面元数据失败: {e}")

    def __get_saved_cover_metadata(self, file_path: str) -> Optional[Dict[str, Any]]:
        history = self.get_data("saved_cover_meta") or {}
        return history.get(str(file_path))

    def __get_service_by_name(self, server_name: str):
        if not server_name:
            return None
        if self._servers and self._servers.get(server_name):
            return self._servers.get(server_name)
        try:
            return self.mediaserver_helper.get_service(name=server_name)
        except Exception:
            return None

    def __find_library_by_id(self, service, library_id: str):
        if not service or not library_id:
            return None
        try:
        ...
        for library in libraries or []:
            current_id = library.get("Id") if service.type == "emby" else library.get("ItemId")
            if str(current_id) == str(library_id):
                return library
        return None

    @staticmethod
    def __read_file_as_base64(file_path: Path) -> Optional[str]:
        try:
    ...
    def __save_image_to_local(self, image_content, server_name: str, library_name: str, extension: str, library_id: str = None):
        """
        保存图片到本地路径
        """
        try:
            ...
            self.__save_cover_file_metadata(
                file_path=file_path,
        ...
        try:
            ...
            if self._save_recent_covers:
                try:
                    ...
                    self.__save_image_to_local(image_bytes, service.name, library['Name'], extension, str(library_id))
    ...
    def __get_fonts(self):
        ...
        default_font_url = {
            ...
            "JosefinSans": "https://raw.githubusercontent.com/sucooer/MoviePilot-Plugins/main/fonts/josefinsans.woff2",
            "LilitaOne": "https://raw.githubusercontent.com/sucooer/MoviePilot-Plugins/main/fonts/lilitaone.woff2",
            "Monoton": "https://raw.githubusercontent.com/sucooer/MoviePilot-Plugins/main/fonts/Monoton.woff2",
            "Plaster": "https://raw.githubusercontent.com/sucooer/MoviePilot-Plugins/main/fonts/Plaster.woff2",
        }
        default_zh_url = default_font_url.get(self._zh_font_preset, "https://raw.githubusercontent.com/sucooer/MoviePilot-Plugins/main/fonts/chaohei.ttf")

        if not self._en_font_preset:
            self._en_font_preset = "EmblemaOne"

        default_en_url = default_font_url.get(self._en_font_preset, "https://raw.githubusercontent.com/sucooer/MoviePilot-Plugins/main/fonts/EmblemaOne.woff2")
